...
export default function UserCard({ user }) {
  return (
    <Link to={`/user/${user.username}`} state={{ id: user.id }} className="text-black block no-underline">
      ...
      <div className="w-32 h-32 mx-auto mb-2 overflow-hidden rounded-full">
        ...
        <div className="font-semibold text-base text-white mt-4">{user.username}</div>
      </div>
    </Link>
  );
}